        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('disposisi.update', $disposisi->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="col-span-1">
                                        ...
                                        <x-text-input id="no_agenda" class="block mt-1 w-full" type="text" name="no_agenda" :value="old('no_agenda', $disposisi->no_agenda)" required autofocus />
                                        <x-input-error :messages="$errors->get('no_agenda')" class="mt-2" />
                                    </div>
                                    <div class="col-span-1">
                                        <x-input-label for="tanggal_agenda" value="Tanggal Agenda *" />
                                        <x-text-input id="tanggal_agenda" class="block mt-1 w-full" type="date" name="tanggal_agenda" :value="old('tanggal_agenda', $disposisi->tanggal_agenda->format('Y-m-d'))" required />
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4 mt-4">
                                    <div class="col-span-1">
                                        ...
                                        <x-text-input id="jenis_agenda" class="block mt-1 w-full" type="text" name="jenis_agenda" :value="old('jenis_agenda', $disposisi->jenis_agenda)" required />
                                    </div>
                                    <div class="col-span-1">
                                        <x-input-label for="sifat" value="Sifat *" />
                                        <x-text-input id="sifat" class="block mt-1 w-full" type="text" name="sifat" :value="old('sifat', $disposisi->sifat)" required />
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4 mt-4">
                                    <div class="col-span-1">
                                        ...
                                        <x-text-input id="nomor_surat" class="block mt-1 w-full" type="text" name="nomor_surat" :value="old('nomor_surat', $disposisi->nomor_surat)" required />
                                    </div>
                                    <div class="col-span-1">
                                        <x-input-label for="tanggal_surat" value="Tanggal Surat *" />
                                        <x-text-input id="tanggal_surat" class="block mt-1 w-full" type="date" name="tanggal_surat" :value="old('tanggal_surat', $disposisi->tanggal_surat->format('Y-m-d'))" required />
                                    </div>
                                </div>

                                <div class="mt-4">
                                    <x-input-label for="asal_surat" value="Asal Surat *" />
                                    <x-text-input id="asal_surat" class="block mt-1 w-full" type="text" name="asal_surat" :value="old('asal_surat', $disposisi->asal_surat)" required />
                                </div>

                                <div class="mt-4">
                                    <x-input-label for="tujuan_surat" value="Tujuan Surat *" />
                                    <x-text-input id="tujuan_surat" class="block mt-1 w-full" type="text" name="tujuan_surat" :value="old('tujuan_surat', $disposisi->tujuan_surat)" required />
                                </div>

                                <div class="mt-4">
                                    <x-input-label for="perihal" value="Perihal *" />
                                    <textarea id="perihal" name="perihal" rows="3" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 rounded-md shadow-sm" required>{{ old('perihal', $disposisi->perihal) }}</textarea>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="col-span-1">
                                        ...
                                        <x-text-input id="jumlah_lembar" class="block mt-1 w-full" type="number" name="jumlah_lembar" :value="old('jumlah_lembar', $disposisi->jumlah_lembar)" required min="1" />
                                    </div>
                                    <div class="col-span-1">
                                        <x-input-label for="lampiran" value="Keterangan Lampiran" />
                                        <x-text-input id="lampiran" class="block mt-1 w-full" type="text" name="lampiran" :value="old('lampiran', $disposisi->lampiran)" />
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4 mt-4">
                                    <div class="col-span-1">
                                        ...
                                        <x-text-input id="klasifikasi" class="block mt-1 w-full" type="text" name="klasifikasi" :value="old('klasifikasi', $disposisi->klasifikasi)" />
                                    </div>
                                    <div class="col-span-1">
                                        <x-input-label for="retensi" value="Retensi" />
                                        <x-text-input id="retensi" class="block mt-1 w-full" type="text" name="retensi" :value="old('retensi', $disposisi->retensi)" />
                                    </div>
                                </div>
                                ...
                                <div class="mt-6">
                                    ...
                                    <div class="mt-2 grid grid-cols-2 gap-2 bg-gray-50 dark:bg-gray-900 p-4 rounded-md border dark:border-gray-700">
                                        @foreach($teams as $team)
                                            <label class="inline-flex items-center">
                                                <input type="checkbox" name="teams[]" value="{{ $team->id }}" {{ (is_array(old('teams', $disposisi->teams->pluck('id')->toArray())) && in_array($team->id, old('teams', $disposisi->teams->pluck('id')->toArray()))) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                ...
                                <div class="mt-4">
                                    ...
                                    <textarea id="catatan" name="catatan" rows="3" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 rounded-md shadow-sm">{{ old('catatan', $disposisi->catatan) }}</textarea>
                                </div>
                                <div class="mt-4">
                                    <x-input-label for="tanggal_disposisi" value="Tanggal Disposisi" />
                                    <x-text-input id="tanggal_disposisi" class="block mt-1 w-full md:w-1/2" type="date" name="tanggal_disposisi" :value="old('tanggal_disposisi', $disposisi->tanggal_disposisi ? $disposisi->tanggal_disposisi->format('Y-m-d') : '')" />
                                </div>
                            </div>
                        </div>
